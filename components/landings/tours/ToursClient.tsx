...
export default function ToursClient() {
  useEffect(() => {
    ...
    // Ticker: pixel-perfect loop (measure first half width so seam is exact)
    (function initTicker() {
    ...
    // Hero load + conv message reveal
    const heroTimer = window.setTimeout(() => {
      const hero = document.querySelector('.hero');
      if (hero) hero.classList.add('hero-loaded');
      const convCard = document.querySelector('.conv-card');
      if (convCard) convCard.classList.add('msg-visible');
    }, 150);
    ...
    // Dashboard: bars, pipeline (run once), metric count-up
    let dashDone = false;
    ...
    // Form submit — sends to Next.js API route
    const form = document.getElementById('auditForm') as HTMLFormElement | null;
    const handleToursSubmit = (e: Event) => {
      e.preventDefault();
        ...
        .then((res) => {
          if (res.ok) {
            if (btn) {
              ...
              btn.textContent = "✓ Audit Request Sent! We'll call within 4 hours.";
              btn.disabled = true;
            }
            form.reset();
          } else {
            if (btn) {
              btn.disabled = false;
              btn.textContent = 'Claim My Free Revenue Leak Audit →';
            }
    ...
    if (form) {
      form.addEventListener('submit', handleToursSubmit);
      cleanups.push(() => form.removeEventListener('submit', handleToursSubmit));
    }
